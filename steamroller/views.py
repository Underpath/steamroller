...
from .steamroller import app, db, oid
...
def all_games(section):
    ...
    app.logger.debug(
        'Listing games in page {} for user "{}" from all.'.format(
            str(section), user["steam_id"]
        ),
        extra=games.get_log_data(),
    )
...
def new_games(section):
    ...
    app.logger.debug(
        'Listing games in page {} for user "{}" from new.'.format(
            str(section), user["steam_id"]
        ),
        extra=games.get_log_data(),
    )
...
def pick():
    ...
    if picks:
        count, game = picks
        app.logger.debug(
            'Picked game {{id: {}, steam_appid: {}}} for user "{}".'.format(
                str(game["id"]), str(game["appid"]), user["steam_id"]
            ),
            extra=games.get_log_data(),
        )
        return render_template(
            "pick.html", my_page=my_page, user=user, game=game, count=count, debug=debug
        )
    else:
        app.logger.error(
            'Error picking game for user "{}".'.format(user["steam_id"]),
            extra=games.get_log_data(),
        )
...
def change_game_preference():
    ...
    if not operation or not game_id:
        app.logger.error(
            'Error while executing operation "{}" with game_id: {} for user "{}": Not enough parameters.'.format(
                operation, str(game_id), user["steam_id"]
            ),
            extra=games.get_log_data(),
        )
        return render_template("error.html", my_page=my_page, user=user)
    if operation == "Add":
        flash(
            'Added "{}" to new games.'.format(game_name.encode("ascii", "ignore")),
            "blue",
        )
    elif operation == "Remove":
        flash(
            'Removed "{}" from new games.'.format(game_name.encode("ascii", "ignore")),
            "blue",
        )
    app.logger.debug(
        'Executing operation "{}" with game_id: {} for user "{}".'.format(
            operation, str(game_id), user["steam_id"]
        ),
        extra=games.get_log_data(),
    )
...
def logout():
    app.logger.debug(
        'Terminating session for user "{}".'.format(g.user.steam_id),
        extra=games.get_log_data(),
    )
...
def create_or_login(resp):
    ...
    app.logger.info(
        'User "{}" authenticated.'.format(g.user.steam_id), extra=games.get_log_data()
    )
...
def generate_list_view(my_games, user, page_details, section):
    ...
    if my_games:
        ...
        app.logger.error(
            'Error while getting games for user "{}".'.format(user["steam_id"]),
            extra=games.get_log_data(),
        )
